...
const Sidebar = () => {
  ...
  const getLinkStyle = (isActive) => {
    const base = `flex items-center ${isCollapsed ? 'justify-center w-12 h-12 p-0' : 'gap-3 px-4 py-2'} rounded-xl font-medium transition-all duration-300 overflow-hidden whitespace-nowrap`;
  ...
  const menuGroups = [
    {
      ...
      items: [
        ...
        { to: "/crawler", title: "Cào Video", icon: DownloadCloud },
        { to: "/discovery", title: "Đang theo dõi", icon: Compass },
        { to: "/live", title: "Live Restream", icon: Bot },
        { to: "/faceless", title: "Sáng tạo AI", icon: Bot, isSpecial: true },
      ]
    },
    {
  ...
  return (
    <motion.aside
      ...
      className={`relative h-full flex flex-col bg-bg-secondary border-r border-border-subtle ${isCollapsed ? 'p-4 items-center' : 'p-5 py-4'} z-40`}
    >
      ...
      {/* Logo Header */}
      <div className={`flex items-center text-2xl font-extrabold text-text-primary mb-5 tracking-tight w-full ${isCollapsed ? 'justify-center' : 'gap-3 px-2'}`}>
        <motion.div
      ...
      {/* Navigation List */}
      <div className="flex-1 flex flex-col gap-3 w-full overflow-y-auto overflow-x-hidden pr-1">
        {menuGroups.map((group, groupIdx) => (
          <div key={groupIdx} className="flex flex-col gap-1 w-full">
            {/* Group Header / Divider */}
            {isCollapsed ? (
              groupIdx > 0 && <div className="h-[1px] bg-border-subtle/30 my-1.5 w-8 mx-auto" />
            ) : (
              <span className="text-[10px] font-bold tracking-wider uppercase text-text-secondary/40 px-3 mb-0.5 block">
                {group.title}
              </span>
            )}

            <ul className={`flex flex-col gap-1 w-full ${isCollapsed ? 'items-center' : ''}`}>
              {group.items.map((item) => {
                if (item.isSpecial) {
                  return (
                    <li className="relative w-full" key={item.to}>
                      <NavLink
                        to={item.to}
                        className={({ isActive }) => `
                      ...
                      >
                        ...
                        <item.icon size={20} className="text-neon-pink drop-shadow-[0_0_8px_rgba(236,72,153,0.8)] group-hover:animate-bounce min-w-[20px]" />
                        <AnimatePresence>
                          {!isCollapsed && (
                            <motion.span
