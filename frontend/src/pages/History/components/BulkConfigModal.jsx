        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 h-[600px] min-h-0">
            ...
            <div className="flex border-b border-white/5 bg-bg-secondary/40 px-4 pt-2 gap-2 overflow-x-auto scrollbar-none">
              {tabs.map(tab => (
                <button
                  ...
                  className={`py-3 px-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-all duration-300 cursor-pointer flex items-center gap-2 pb-3 whitespace-nowrap ${
                    activeTab === tab.id
          ...
          <div className="lg:col-span-2 flex flex-col gap-4 bg-bg-primary/20 rounded-2xl border border-white/5 p-5 h-full min-h-0 glass-panel">
            <label className="block text-xs font-bold text-text-secondary uppercase tracking-wider flex justify-between items-center">
              <span className="flex items-center gap-1.5"><PlayCircle size={14} className="text-brand-primary animate-pulse" /> Preview Hiệu ứng & Phụ đề</span>
              <span className="text-[10px] text-brand-primary normal-case font-semibold">Kéo thả phụ đề trên ảnh để chỉnh vị trí</span>
            </label>
