...
### 2. Cào Video Douyin & Tự động Vượt rào (Anti-Bot)
1. Chuyển sang Tab **🕷️ Cào Video**.
2. Nhập Link một Video Douyin/TikTok (hoặc Profile) bạn muốn cào. Video sẽ tự động tải xuống không có logo (watermark).
3. **Cơ chế chống chặn Douyin:** Hệ thống đã tích hợp thuật toán sinh mã `a_bogus` và tự động bóc tách dấu vân tay trình duyệt (`User-Agent`). Nếu bị chặn ngầm (như yêu cầu Captcha), hệ thống sẽ tự động kích hoạt trình duyệt tàng hình (Playwright) chạy ngầm để lấy Cookie mới và tự động tải tiếp, hoặc dùng giải pháp bóc tách HTML gốc (HTML Fallback) cực kỳ lì đòn.
...
### 4. Font Chữ Tùy Chỉnh & Watermark (Chữ ký/Logo)
*Hệ thống cho phép bạn tùy biến Font chữ cho cả Phụ đề và Watermark để video mang đậm tính cá nhân.*
1. Mở thư mục `data/fonts` trong thư mục gốc dự án.
2. Copy các file font chữ yêu thích của bạn có đuôi `.ttf` hoặc `.otf` (VD: `Montserrat-Bold.ttf`, `BeVietnamPro.ttf`) vào thư mục này.
3. Khi F5 lại giao diện Web, hệ thống sẽ tự động quét thư mục và đưa các Font chữ này vào danh sách "Font chữ" thả xuống trong Menu cấu hình Subtitle.
4. Chọn Font và tùy chỉnh Cỡ chữ, Độ dày nền lượn sóng (Wavy background) rồi tiến hành Render!

### 5. Upload tự động (GPM Anti-Detect)
