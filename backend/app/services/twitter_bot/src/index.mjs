...
const RUN_LOG = process.env.RUN_LOG_PATH || 'data/run.log';
...
async function main() {
  ...
  const storePath = process.env.STORE_PATH || 'data/store.db';
  initStore(storePath);
  ...
  if (!process.argv.includes('--run-once')) {
    ...
    log(`Skipping warmup loop in run-once mode.`);
  }
  ...
  while (true) {
    ...
    if (process.argv.includes('--run-once')) {
      log(`Run-once flag detected. Exiting cycle.`);
      process.exit(0);
    }
