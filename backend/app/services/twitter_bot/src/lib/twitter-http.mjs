...
function getAgent() {
  return process.env.PROXY_URL ? new HttpsProxyAgent(process.env.PROXY_URL) : undefined;
}
...
function httpGet(url, headers) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(
        { hostname: parsed.hostname, path: parsed.pathname + parsed.search, headers, agent: getAgent() },
        (res) => {
...
function httpRequest(method, hostname, apiPath, headers, body) {
  return new Promise((resolve, reject) => {
    const req = https.request(
      { hostname, path: apiPath, method, headers, timeout: 60000, agent: getAgent() },
      (res) => {
        let data = '';
        res.on('data', (d) => (data += d));
